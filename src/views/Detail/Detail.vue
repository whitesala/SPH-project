  ...
  <div class="detail">
    ...
    <section class="con">
      ...
      <div class="conPoin">
        ...
        <span v-if="categoryView.category4Id">{{ categoryView.category34Name }}系类</span>
      </div>
      <!-- 主要内容区域 -->
      <div class="mainCon">
        <!-- 左侧放大镜区域 -->
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :goodsImgList="goodsImgList"/>
          <!-- 小图列表 -->
          <!-- 向ImageList组件传值 -->
          <ImageList :goodsImgList="goodsImgList"/>
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <!-- 这里的itemValue是当前添加了active类的选项 -->
                <dd changepirce="0"
                :class="{active:itemValue.isChecked === '1'}" v-for="itemValue in item.spuSaleAttrValueList" :key="itemValue.id"
                @click="changeActive(itemValue,item.spuSaleAttrValueList)">{{ itemValue.saleAttrValueName }}</dd>
              </dl>
              <dl >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @change="purchaseNum">
...
export default {
  ...
  methods: {
    // 改变选中的状态，使用排他思想
    changeActive (selectItem, allItem) {
      allItem.forEach(item => {
        // this.isChecked = '0'
        // 根据选中元素是否为当前的元素，是则为1，否则修改为0表示未选中
        item.isChecked = item === selectItem ? '1' : '0'
        // console.log(item.isChecked)
      })
      // selectItem.isChecked = '1'
    },
    // 用户自己输入购买的商品数量
    purchaseNum (event) {
      ...
      if (isNaN(numValue) || numValue <= 0) {
        ...
        this.skuNum = 99
        alert('购买数量过大!最高单次购买量为99件')
      } else {
        this.skuNum = Math.ceil(numValue)
      }
    }
  }
}
